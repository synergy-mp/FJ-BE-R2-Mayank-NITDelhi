...
async function checkBudgetAndNotify(userId, categoryId) {
  try {
    ...
    const txns = await prisma.transaction.findMany({ where: { userId, categoryId, type: "EXPENSE" } });
    let totalSpentUSD = 0;
    txns.forEach(t => { totalSpentUSD += convertCurrency(parseFloat(t.amount), t.currency, "USD"); });
    ...
    if (totalSpentUSD > parseFloat(budget.limit)) {
      const mailOptions = {
        ...
        text: `Hello ${budget.user.name}, you exceeded your budget for "${budget.category.name}".`
      };
